...
任务 `BPX-Stand-Rough-v0` 与 `BPX-Stand-v0` 共用 10 帧非对称观测、12 维动作、零速度指令和 PPO 网络。Actor 每帧 45 维（无机身线速度），Critic 每帧 48 维（含线速度真值）；历史按观测项展开为 450/480 维。日志独立写入 `logs/rsl_rl/bpx_rough_stand/`。
...
旧版 48 维 Stand checkpoint 的网络输入与新任务不匹配，需从头训练。以后仅能从相同 450/480 维观测配置的 checkpoint 恢复训练；地形课程状态不随 PPO checkpoint 保存。
...
- 增加左右关节镜像惩罚（权重 -1.0）：髋外展偏移左右反号，髋俯仰和膝关节偏移左右同号；误差在 0.10 rad 死区内不惩罚，以保留崎岖地形上的必要补偿。超出死区的误差先按 0.20 rad 归一化再平方，避免原始 `rad²` 数值过小导致该项实际不起作用。
- 增加整机质心居中奖励（权重 +1.0）：按所有刚体质量计算整机质心，并约束其水平投影接近四足几何中心；2 cm 内满奖励，超过后按 5 cm 尺度指数衰减。该项直接约束支撑几何，不能由关节角镜像替代。
...
每回合 15 秒，前 0.5 秒为落脚稳定期。此后满足下列稳定条件的时间占比达到 90%，四足同时接触占比达到 90%，左右对称占比和质心居中占比均达到 80%，累计考察时间至少 14.4 秒且没有物理失败，视为成功：
...
- 四足同时接触且整机质心投影距四足几何中心 ≤0.05 m，才计为该时刻质心居中。
...
观察课程指标 `level`、`sampled_level`、`success_rate`、`stable_fraction`、`four_feet_fraction`、`symmetry_fraction`、`symmetry_error`、`support_center_fraction`、`support_center_error`、`max_drift`；除 `level` 外均统计本次重置批次，并非整场训练的累计指标。更改回合长度时同时调整 `min_duration`。
...
导出的策略位于该 run 的 `exported/policy.onnx`。在 `sim2sim/config/bpx_terrain_history.toml` 修改 `stand_policy` 为该路径，保留已有的 `locomotion_policy` 和 Init 控制器配置：
...
检查真实地面查询、分层/切换状态重置、四足接触课程、高度奖励、450/480 维观测与 12 维动作兼容性、升降级、最高级钳制、漂移失败和有限值物理步进。短验证不代表策略已学会崎岖站立。
