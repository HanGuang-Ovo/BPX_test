...
独立的 H=10 历史版本见 [ROUGH_HISTORY_TRAINING.md](ROUGH_HISTORY_TRAINING.md)，任务名为 `BPX-Locomotion-Rough-History-v0`。
...
连续两次成功升一级；连续两次失败降一级（降级滞回）。失败指物理终止，或回合持续至少 1 秒但平均跟踪误差超限。
单次失败只累计不降级，任何非失败回合都会清空失败连续计数。
