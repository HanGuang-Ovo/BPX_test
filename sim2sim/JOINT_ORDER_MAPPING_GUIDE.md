...
- [ ] Isaac Lab 日志打印的 12 个动作关节与标准表一致；
- [ ] `validate_setup.py` 打印的 12 个关节逐项一致；
- [ ] 默认关节角数组采用同一顺序；
- [ ] `joint_pos_rel`、`joint_vel_rel` 和 `last_action` 使用同一顺序；
- [ ] 12 个 one-hot 动作均控制预期关节；
- [ ] Isaac 与 MuJoCo 的正关节方向一致；
- [ ] ONNX 输入为 48 维、输出为 12 维；
- [ ] 策略文件旁保存了完整版本和关节元数据；
- [ ] 模型或机器人资产发生变化后重新完成上述检查。
