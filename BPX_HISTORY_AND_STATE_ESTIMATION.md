...
实现更新：现已新增独立的 `BPX-Locomotion-Rough-History-v0`，使用 H=10、48 维完整观测历史，Actor/Critic 共用 480 维输入。训练及部署契约见 [使用说明](ROUGH_HISTORY_TRAINING.md)。下文的单帧工程现状描述对应原基线；非对称观测、历史编码器与速度估计器仍为后续方案。
